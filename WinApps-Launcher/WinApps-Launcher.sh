...
declare -rx APPDATA_PATH="${HOME}/.local/share/winux"
declare -rx CONFIG_PATH="${HOME}/.config/winux"
declare -rx CONFIG_FILE="${CONFIG_PATH}/winux.conf"
declare -rx COMPOSE_FILE="${CONFIG_PATH}/compose.yaml"
declare -rx USER_WINUX_APPLICATIONS="${HOME}/.local/share/winux/apps"
declare -rx SYSTEM_WINUX_APPLICATIONS="/usr/local/share/winux/apps"
...
function winux_flavor_detection() {
...
function kill_freerdp() {
    ...
    for FREERDP_PROCESS_FILE in "${APPDATA_PATH}/FreeRDP_Process_"*.cproc; do
        ...
        if [ -f "$FREERDP_PROCESS_FILE" ]; then
            ...
            # Remove the file.
            # NOTE: This is not necessary as 'bin/winux' will automatically delete the file once the process terminates.
...
function app_select() {
    if check_reachable; then
        ...
        # Ignore files that do not contain "${WINUX_PATH}/winux".
        # Ignore files named "winux" and "windows".
        for FILE in "${ALL_FILES[@]}"; do
            if grep -q "${WINUX_PATH}/winux" "$FILE" && [ "$(basename "$FILE")" != "windows" ] && [ "$(basename "$FILE")" != "winux" ]; then
                # Store the filename.
...
# 'winux'
if ! command -v winux &> /dev/null; then
    show_error_message "ERROR: 'winux' <u>NOT FOUND</u>.\nPlease ensure 'winux' is installed."
    exit "$EC_MISSING_DEP"
else
    WINUX_PATH=$(dirname "$(which winux)")
fi

# INITIALISATION.
check_config_exists
winux_flavor_detection
