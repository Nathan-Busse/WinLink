...
Although Winux supports using `QEMU+KVM+libvirt` as a backend for running Windows virtual machines, it is recommended to use `Docker` or `Podman`. These backends automate the setup process, eliminating the need for manual configuration and optimisation of the Windows virtual machine.
...
> [!IMPORTANT]
> Winux does __NOT__ officially support versions of Windows prior to Windows 10. Despite this, it may be possible to achieve a successful installation with some additional experimentation. If you find a way to achieve this, please share your solution through a pull request for the benefit of other users.
...
Winux utilises `docker compose` to configure Windows VMs. A template [`compose.yaml`](../compose.yaml) is provided.
...
docker compose --file ~/.config/winux/compose.yaml down
...
docker compose --file ~/.config/winux/compose.yaml up
```

### Subsequent Use
```bash
docker compose --file ~/.config/winux/compose.yaml start # Power on the Windows VM
docker compose --file ~/.config/winux/compose.yaml pause # Pause the Windows VM
docker compose --file ~/.config/winux/compose.yaml unpause # Resume the Windows VM
docker compose --file ~/.config/winux/compose.yaml restart # Restart the Windows VM
docker compose --file ~/.config/winux/compose.yaml stop # Gracefully shut down the Windows VM
docker compose --file ~/.config/winux/compose.yaml kill # Force shut down the Windows VM
```
...
> [!IMPORTANT]
> Ensure `WAFLAVOR` is set to `"podman"` in `~/.config/winux/winux.conf`.
...
```bash
# Stop and remove the existing container.
podman-compose --file ~/.config/winux/compose.yaml down
...
# Re-create the container with the updated configuration.
podman-compose --file ~/.config/winux/compose.yaml up
```

### Subsequent Use
```bash
podman-compose --file ~/.config/winux/compose.yaml start # Power on the Windows VM
podman-compose --file ~/.config/winux/compose.yaml pause # Pause the Windows VM
podman-compose --file ~/.config/winux/compose.yaml unpause # Resume the Windows VM
podman-compose --file ~/.config/winux/compose.yaml restart # Restart the Windows VM
podman-compose --file ~/.config/winux/compose.yaml stop # Gracefully shut down the Windows VM
podman-compose --file ~/.config/winux/compose.yaml kill # Force shut down the Windows VM
```
