...
declare -rx APPDATA_PATH="${HOME}/.local/share/winlink"
declare -rx CONFIG_PATH="${HOME}/.config/winlink"
declare -rx CONFIG_FILE="${CONFIG_PATH}/winlink.conf"
declare -rx COMPOSE_FILE="${CONFIG_PATH}/compose.yaml"
declare -rx USER_WINUX_APPLICATIONS="${HOME}/.local/share/winlink/apps"
declare -rx SYSTEM_WINUX_APPLICATIONS="/usr/local/share/winlink/apps"
...
function kill_freerdp() {
    ...
    for FREERDP_PROCESS_FILE in "${APPDATA_PATH}/FreeRDP_Process_"*.cproc; do
        ...
        if [ -f "$FREERDP_PROCESS_FILE" ]; then
            ...
            # Remove the file.
            # NOTE: This is not necessary as 'bin/winlink' will automatically delete the file once the process terminates.
...
function app_select() {
    if check_reachable; then
        ...
        # Store the paths of all files within the directory 'WINUX_PATH'.
        readarray -t ALL_FILES < <(find "$WINUX_PATH" -maxdepth 1 -type f)

        # Ignore files that do not contain "${WINUX_PATH}/winlink".
        # Ignore files named "winlink" and "windows".
        for FILE in "${ALL_FILES[@]}"; do
            if grep -q "${WINUX_PATH}/winlink" "$FILE" && [ "$(basename "$FILE")" != "windows" ] && [ "$(basename "$FILE")" != "winlink" ]; then
                # Store the filename.
...
# 'winlink'
if ! command -v winlink &> /dev/null; then
    show_error_message "ERROR: 'winlink' <u>NOT FOUND</u>.\nPlease ensure 'winlink' is installed."
    exit "$EC_MISSING_DEP"
else
    WINUX_PATH=$(dirname "$(which winlink)")
fi

# INITIALISATION.
check_config_exists
winlink_flavor_detection
